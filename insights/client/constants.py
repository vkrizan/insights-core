...
def _log_dir():
    if _uid == 0:
        return os.path.join(os.sep, 'var', 'log', _app_name)
    else:
        local_log_dir = os.getenv('XDG_CACHE_HOME', default=os.path.join(_user_home, '.cache'))
        local_insights_log_dir = os.path.join(local_log_dir, _app_name)
        try:
            os.makedirs(local_insights_log_dir)
        except:
            pass
        return local_insights_log_dir


def _lib_dir():
    if _uid == 0:
        return os.path.join(os.sep, 'var', 'lib', 'insights')
    else:
        local_lib_dir = os.path.join(_user_home, '.local', 'lib')
        local_insights_lib_dir = os.path.join(local_lib_dir, _app_name)
        try:
            os.makedirs(local_insights_lib_dir)
        except:
            pass
        return local_insights_lib_dir
